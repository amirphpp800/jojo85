...
function renderEntries(entries) {
  ...
  if (entries.length === 0) {
    dnsGrid.innerHTML = '<div class="empty-state">هنوز هیچ DNS ثبت نشده است</div>';
    return;
  }
...
async function fixCountryNames() {
  ...
  try {
    const res = await fetch('/api/admin/fix-country-names', { method: 'POST' });
...
async function removeDuplicates() {
  ...
  try {
    const res = await fetch('/api/admin/remove-duplicates', { method: 'POST' });
    const data = await res.json();
...
async function downloadJSON() {
  try {
    ...
    a.href = url;
    a.download = `dns-addresses-${new Date().toISOString().split('T')[0]}.json`;
  ...
  bulkForm.addEventListener('submit', async (e) => {
    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      try {
        ...
        const res = await fetch('/api/admin/bulk-add', {
          method: 'POST',
          body: formData
        });
